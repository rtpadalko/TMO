...
def preprocess_data(data_in):
    ...
    temp_y = data_out['quality']
    # Добавляем фиктивный второй класс
    temp_y_dummy = temp_y.copy()
    temp_y_dummy[:] = 0  # Задаем значение фиктивного класса
    # Соединяем данные с фиктивным классом
    temp_X = pd.concat([temp_X, temp_X])
    temp_y = pd.concat([temp_y, temp_y_dummy])
    # Разбиваем на обучающий и тестовый наборы
    X_train, X_test, y_train, y_test = train_test_split(temp_X, temp_y, train_size=0.8, random_state=1)
...
def print_models(models_select, X_train, X_test, y_train, y_test):
    ...
    for model_name in models_select:
        ...
        cm = confusion_matrix(y_test, model.predict(X_test), normalize='all', labels=model.classes_)
