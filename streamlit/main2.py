...
def preprocess_data(data_in):
    ...
    data_out['quality'] = data_out['quality']
...
if st.checkbox('Показать корреляционную матрицу для числовых признаков'):
    # Создаем новый DataFrame, исключив столбцы с текстовыми значениями
    numeric_data = data.select_dtypes(include=['float64', 'int64'])

    # Построим тепловую карту корреляции для числовых столбцов
    fig1, ax = plt.subplots(figsize=(10,5))
    sns.heatmap(numeric_data.corr(), annot=True, fmt='.2f')
    st.pyplot(fig1)


cv_slider = st.slider('Количество фолдов (частей):', min_value=3, max_value=10, value=5, step=1)
...
st.write('Максимальное допустимое количество ближайших соседей с учетом выбранного количества фолдов (частей) - {}'.format(allowed_knn))
...
st.write('Значения accuracy для отдельных фолдов (частей)')
st.bar_chart(scores)
st.write('Усредненное значение accuracy по всем фолдам (частям) - {}'.format(np.mean(scores)))